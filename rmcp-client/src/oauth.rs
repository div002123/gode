...
//!
//! If the keyring is not available or fails, we fall back to FREECODE_HOME/.credentials.json which is consistent with other coding CLI agents.
...
pub enum OAuthCredentialsStoreMode {
    ...
    #[default]
    Auto,
    /// FREECODE_HOME/.credentials.json
    /// This file will be readable to FreeCode and other applications running as the same user.
    File,
...
mod tests {
    ...
    impl TempCodexHome {
        fn new() -> Self {
            ...
            let dir = tempdir().expect("create FREECODE_HOME temp dir");
            unsafe {
                std::env::set_var("FREECODE_HOME", dir.path());
            }
            Self {
                _guard: guard,
                _dir: dir,
    ...
    impl Drop for TempCodexHome {
        fn drop(&mut self) {
            unsafe {
                std::env::remove_var("FREECODE_HOME");
            }
        }
    }
